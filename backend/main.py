...
def process(job_id, url):
    ...
    try:
        ...
        run(["yt-dlp", "--no-playlist", "-f", "bv*+ba/b", "--merge-output-format", "mp4", "-o", str(work / "source.%(ext)s"), url])
        ...
        # Fallback candidate windows guarantee actual playable clips.
        ...
        for i,(s,e) in enumerate(windows,1):
            ...
            clips.append({"title":f"Viral Candidate #{i}","duration":e-s,"start":s,"score":max(70,98-i*5),"source":url,"resolution":"1080×1920","url":f"/files/{out.name}"})
...
def analyze(r: Req):
    ...
    threading.Thread(target=process,args=(job,r.url),daemon=True).start()
    return {"job_id":job}
